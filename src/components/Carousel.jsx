...
const Carousel = ({ items }) => {
  ...
  return (
    <div className="relative w-full overflow-hidden">
      ...
      <button
        className="absolute left-20 top-1/2 transform -translate-y-1/2 bg-white border-none w-8 h-8"
        onClick={prevSlide}
      >
        <Image src="/left-icon.png" width={20} height={20} alt="left icon" className="ml-1" />
      </button>
      <button
        className="absolute right-20 top-1/2 transform -translate-y-1/2 bg-white border-none w-8 h-8"
        onClick={nextSlide}
      >
